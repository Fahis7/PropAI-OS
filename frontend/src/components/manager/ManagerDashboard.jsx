      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center text-gray-400">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Ask your admin to assign you a property.
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">
                PropOS <span className="text-blue-400">Manager</span>
              </h1>
          ...
          <>
            ...
            {stats.emergency > 0 && (
              <div className="bg-red-900/30 border border-red-500 p-4 rounded-xl flex items-center gap-3 animate-pulse">
                ...
                <p className="font-bold text-red-300">
                  🚨 {stats.emergency} Emergency Ticket
                  {stats.emergency > 1 ? "s" : ""} — Immediate action required!
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard
                ...
                sub={`${stats.occupied}/${stats.total_units} units`}
                color="blue"
              />
              <StatCard
                label="Revenue"
              ...
              <StatCard
                label="Pending"
                value={`AED ${stats.pending.toLocaleString()}`}
                sub={`${stats.bounced} bounced`}
                color="yellow"
              />
              <StatCard
                label="Open Tickets"
                value={stats.open_tickets + stats.in_progress}
                sub={`${stats.open_tickets} open, ${stats.in_progress} active`}
                color="orange"
              />
            </div>
                    ...
                    >
                      ...
                      <div className="w-full bg-gray-700 h-1 rounded mt-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(t.active_tickets * 25, 100)}%`,
                          }}
                        />
                      </div>
                    </div>
                ...
                {recent_tickets.slice(0, 5).map((t) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        Unit {t.unit_number} • {t.created_at}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0 ml-3">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getPriorityColor(t.priority)}`}
                      >
                        {t.priority}
                      </span>
                      <span
        ...
        {tab === "tickets" && (
          <div className="space-y-2">
            <h3 className="text-lg font-bold text-white mb-3">
              All Maintenance Tickets — {property.name}
            </h3>
              ...
              {units.map((u) => (
                ...
                >
                  ...
                  <p className="text-sm font-bold text-blue-400 mt-1">
                    AED {u.yearly_rent.toLocaleString()}/yr
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  These rules are shared with the AI chatbot. Tenants can ask
            ...
            {rules.length > 0 && (
              <div className="mt-4 bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
                <p className="text-xs text-blue-400">
                  💡 Tenants can now ask the AI chatbot questions like "Can I
                  keep a cat?" or "What are the gym hours?" and it will answer
                  using these rules.
                </p>
              </div>
            )}
...
const StatCard = ({ label, value, sub, color }) => {
  ...
  return (
    <div className={`bg-gray-800 rounded-xl border ${colors[color]} p-4`}>
      ...
      <p className={`text-xl font-bold mt-1 ${colors[color]?.split(" ")[1]}`}>
        {value}
      </p>
