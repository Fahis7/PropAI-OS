...
const TenantNav = ({ notificationCount = 0 }) => {
    ...
    return (
        <nav className={'fixed bottom-0 left-0 right-0 z-50 border-t ' + (isDark ? 'bg-[#0d1117] border-white/5' : 'bg-white border-gray-200')}>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button key={item.path} onClick={() => navigate(item.path)}
                            className={'flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all relative ' + (isActive ? c.accent : c.textMut)}>
                            ...
                            <span className={'text-[10px] font-medium ' + (isActive ? c.accent : '')}>{item.label}</span>
                            {isActive && <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-amber-500 rounded-full" />}
                        </button>
                    );
                })}
            </div>
