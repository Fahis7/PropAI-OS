...
def analyze_rent_price(unit, retry=False):
    """
    Smart Rent Pricing Engine — Uses Groq Llama AI (FREE) + Dubai market data
    ...
    if not GROQ_API_KEY:
        print("⚠️ No Groq API key — returning market data only")
    ...
    try:
        ...
        prompt = f"""You are an expert Dubai real estate rental analyst. Analyze this unit and provide a smart rent recommendation.
...
{{"recommended_low": <number>, "recommended_mid": <number>, "recommended_high": <number>, "confidence": <number 1-100>, "verdict": "<UNDERPRICED|FAIR|OVERPRICED|PREMIUM>", "reasoning": "<2-3 sentences>", "tips": ["<tip 1>", "<tip 2>", "<tip 3>"]}}"""
        ...
        print(f"🤖 Groq Pricing Response: {text}")
        ...
        if "429" in str(e) and not retry:
            print("⚠️ Rate limit. Retrying in 10s...")
            time.sleep(10)
            return analyze_rent_price(unit, retry=True)

        print(f"❌ Groq Pricing Failed: {e}")
        return build_fallback_response(unit_info, market_info)
...
def build_fallback_response(unit_info, market_info):
    ...
    if current < min_rent:
        ...
        verdict = "PREMIUM"
        reasoning = f"Current rent (AED {current:,.0f}) is above market average (AED {avg:,}). Sustainable if the unit has premium features."
    else:
        verdict = "FAIR"
